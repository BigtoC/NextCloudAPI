...
class Share:
    ...
    def createShare(self, path, shareType, shareWith=None, publicUpload=None, password=None, permissions=None):
        ...
        if publicUpload is True:
            publicUpload = "true"
        if (path is None or isinstance(shareType, int) is not True) or (
                shareType in [0, 1] and shareWith is None):
            return False
        msg = {"path": path, "shareType": shareType}
        if shareType in [0, 1]: msg["shareWith"] = shareWith
        if publicUpload is True: msg["publicUpload"] = publicUpload
        if shareType == 3 and password is not None: msg["password"] = str(password)
        if permissions is None: msg["permissions"] = permissions
...
class Apps:
    ...
    def getApps(self, filter=None):
        if filter is True:
            return self.get(Apps.url + "?filter=enabled" + self.tojs)
        elif filter is False:
            return self.get(Apps.url + "?filter=disabled" + self.tojs)
...
class User:
    ...
    def editUser(self, uid, email=None, quota=None, displayname=None, phone=None, address=None, website=None,
                 twitter=None, password=None):
...
class NextCloud(Req, User, Group, Apps, Share, GroupFolders):
    ...
    def __init__(self, endpoint, user, pwd, js=False):
        if js is True:
            self.tojs = "?format=json"
        elif js is False:
            self.tojs = ""
